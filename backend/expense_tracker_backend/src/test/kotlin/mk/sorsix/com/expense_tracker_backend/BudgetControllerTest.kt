...
class BudgetControllerTest {
    ...
    fun `update nonexistent budget returns not found`() {
        ...
        mockMvc.put("/api/budgets/999999") {
            contentType = MediaType.APPLICATION_JSON
            header("Authorization", "Bearer ${user.accessToken}")
    ...
    fun `user cannot update another users budget`() {
        ...
        val createResponse = mockMvc.post("/api/budgets") {
            contentType = MediaType.APPLICATION_JSON
        ...
        mockMvc.put("/api/budgets/$budgetId") {
            contentType = MediaType.APPLICATION_JSON
    ...
    fun `delete budget succeeds for owner`() {
        ...
        mockMvc.delete("/api/budgets/$budgetId") {
            header("Authorization", "Bearer ${user.accessToken}")
    ...
    fun `actualSpent reflects logged expenses`() {
        ...
        mockMvc.get("/api/budgets?month=2026-08-01") {
            header("Authorization", "Bearer ${user.accessToken}")
        }.andExpect {
            status { isOk() }
