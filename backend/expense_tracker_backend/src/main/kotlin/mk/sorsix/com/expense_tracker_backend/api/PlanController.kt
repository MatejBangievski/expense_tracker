...
class PlanController(
    ...
    ): List<PlanResponse> = planService.listPlans(currentUserProvider.resolve(userDetails))
    ...
    fun createPlan(
        ...
        when (val result = planService.createPlan(currentUserProvider.resolve(userDetails), request)) {
            ...
            is CreatePlanResult.InvalidDateRange ->
                ResponseEntity
    ...
    fun updatePlan(
        ...
        when (val result = planService.updatePlan(currentUserProvider.resolve(userDetails), id, request)) {
            ...
            is UpdatePlanResult.PlanNotFound ->
                ResponseEntity
                    .status(HttpStatus.NOT_FOUND)
                    .body(mapOf("error" to "Plan not found"))

            is UpdatePlanResult.NotOwner ->
                ResponseEntity
                    .status(HttpStatus.FORBIDDEN)
                    .body(mapOf("error" to "You do not own this plan"))

            is UpdatePlanResult.InvalidDateRange ->
                ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(mapOf("error" to "End date cannot be before start date"))
        }
    ...
    fun deletePlan(
        ...
        when (val result = planService.deletePlan(currentUserProvider.resolve(userDetails), id)) {
            ...
            is DeletePlanResult.PlanNotFound ->
                ResponseEntity
                    .status(HttpStatus.NOT_FOUND)
                    .body(mapOf("error" to "Plan not found"))

            is DeletePlanResult.NotOwner ->
                ResponseEntity
                    .status(HttpStatus.FORBIDDEN)
                    .body(mapOf("error" to "You do not own this plan"))
        }
