...
class CategoryController(
    ...
    @GetMapping
    fun listAllCategories(@AuthenticationPrincipal userDetails: UserDetails): List<CategoryResponse> =
        categoryService.listAllCategories(currentUserProvider.resolve(userDetails))
    ...
    fun createCategory(
        ...
        when (val result = categoryService.createCategory(currentUserProvider.resolve(userDetails), request)) {
            ...
            CreateCategoryResult.NameAlreadyExists -> ResponseEntity.status(HttpStatus.CONFLICT)
                .body(mapOf("error" to "Category name already exists"))

            CreateCategoryResult.ParentCategoryNotFound -> ResponseEntity.status(HttpStatus.BAD_REQUEST)
    ...
    fun update(
        ...
        when (val result = categoryService.updateCategory(currentUserProvider.resolve(userDetails), id, request)) {
            ...
            is UpdateCategoryResult.CategoryNotFound -> ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(mapOf("error" to "Category not found"))

            is UpdateCategoryResult.ParentCategoryNotFound -> ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(mapOf("error" to "Parent category not found"))

            is UpdateCategoryResult.NotOwner -> ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(mapOf("error" to "You do not own this category"))

            is UpdateCategoryResult.CannotEditDefault -> ResponseEntity.status(HttpStatus.FORBIDDEN)
    ...
    fun delete(
        ...
        when (val result = categoryService.deleteCategory(currentUserProvider.resolve(userDetails), id)) {
            ...
            is DeleteCategoryResult.CategoryNotFound -> ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(mapOf("error" to "Category not found"))

            is DeleteCategoryResult.NotOwner -> ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(mapOf("error" to "You do not own this category"))

            is DeleteCategoryResult.CannotDeleteDefault -> ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(mapOf("error" to "Default categories cannot be deleted"))

            is DeleteCategoryResult.InUse -> ResponseEntity.status(HttpStatus.CONFLICT)
