...
class UserChatClientProvider(
    ...
    fun forUser(user: User): ChatClient =
        user.aiApiKey?.let { clientFor(encryptionService.decrypt(it)) } ?: defaultChatClient

    fun validate(apiKey: String): Boolean = try {
        clientFor(apiKey).prompt().user("ping").call().content()
        true
    } catch (ex: Exception) {
        false
    }
