...
class MonthlySavingService(
    ...
    fun findByUserAndMonth(
        userId: Long,
        month: LocalDate,
    ): MonthlySaving? = monthlySavingRepository.findByUserIdAndSavingMonth(userId, month)
    ...
    fun generateWithAIAndPersist(
        ...
        return GenerateMonthlySavingPlanResult.Success(
            persist(
                ...
                totalIncome,
                currentSummary,
                aiResult.recommendationMessage,
                aiResult.monthlyPlan?.categoryLimits ?: emptyList(),
            ),
        )
    }
    ...
    fun createManualPlan(
        user: User,
        request: ManualSavingPlanRequest,
    ): GenerateMonthlySavingPlanResult {
        val savingMonth = LocalDate.now(clock).withDayOfMonth(1)
    ...
    private fun persist(
        ...
        categoryLimits: List<CategoryBudgetSuggestion>,
    ): MonthlySavingPlanResponse {
        ...
        val existingBudgetsByCategoryId =
            budgetRepository
                .findByMonthlySavingId(savedSaving.id)
                .associateBy { it.category.id }

        val budgets =
            allNames.mapNotNull { name ->
    ...
    private fun MonthlySaving.toResponse(budgets: List<Budget>) =
        MonthlySavingPlanResponse(
            id = id,
    ...
    private fun buildPrompt(
        previousSummary: PeriodSummaryResponse?,
        currentSummary: PeriodSummaryResponse,
        savingMonth: LocalDate,
        budgetLimit: BigDecimal,
    ): String =
        buildString {
