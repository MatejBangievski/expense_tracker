...
class MonthlySavingController(
    ...
    fun current(
        ...
        monthlySavingService
            .getCurrentPlan(currentUserProvider.resolve(userDetails))
    ...
    ): ResponseEntity<*> = monthlySavingService.createManualPlan(currentUserProvider.resolve(userDetails), request).toResponse()
    ...
    fun generate(
        ...
        monthlySavingService
            .generateWithAIAndPersist(
                user = currentUserProvider.resolve(userDetails),
                budgetLimit = request.budgetLimit,
                totalIncome = request.totalIncome,
            ).toResponse()

    private fun GenerateMonthlySavingPlanResult.toResponse(): ResponseEntity<*> =
        when (this) {
            is GenerateMonthlySavingPlanResult.Success -> ResponseEntity.ok(savingPlan)
            is GenerateMonthlySavingPlanResult.InsufficientData ->
                ResponseEntity
                    .status(HttpStatus.UNPROCESSABLE_ENTITY)
                    .body(mapOf("error" to message))
            is GenerateMonthlySavingPlanResult.AiUnavailable ->
                ResponseEntity
