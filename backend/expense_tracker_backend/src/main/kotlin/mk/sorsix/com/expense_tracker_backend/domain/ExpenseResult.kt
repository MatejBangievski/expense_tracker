...
sealed class RecentExpensesResult {
    data class Success(val expenses: List<ExpenseResponse>) : RecentExpensesResult()
    object InvalidLimit : RecentExpensesResult()
}

sealed class TotalSpentResult {
    data class Success(val total: TotalResponse) : TotalSpentResult()
    object InvalidRange : TotalSpentResult()
}

sealed class TopCategoryResult {
    data class Success(val topCategory: TopCategoryResponse) : TopCategoryResult()
