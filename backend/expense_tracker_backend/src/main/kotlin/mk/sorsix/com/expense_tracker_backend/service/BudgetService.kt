...
class BudgetService(
    ...
    fun listBudgetsByMonth(
        user: User,
        month: LocalDate,
    ): List<BudgetResponse> {
        val monthlySaving =
            monthlySavingService.findByUserAndMonth(user.id, month)
                ?: return emptyList()
    ...
    private fun refreshActualSpent(
        budget: Budget,
        month: LocalDate,
    ): Budget {
        ...
        val spent =
            expenseRepository.sumAmountByUserAndCategoryAndDateRange(
                budget.user.id,
                budget.category.id,
                start,
                end,
    ...
    private fun Budget.toResponse(month: LocalDate) =
        BudgetResponse(
            id = id,
